  ...
  return (
    <div className="w-full">
      <div className="flex justify-between">
        <h1 className="text-2xl font-bold mb-4 text-sky-500">All Vehicles</h1>
      {error && <div className="text-red-500 mb-2">{error}</div>}
      {user?.role === "user" && (
        <Button onClick={handleAdd} className="mb-4 bg-sky-500">Add Vehicle</Button>
      )}</div>
      ...
      {showForm && user?.role === "user" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/30 backdrop-blur-xs">
          <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md w-full max-w-md relative">
            ...
            <h2 className="text-lg font-semibold mb-4 text-sky-500">{editing ? "Edit Vehicle" : "Add Vehicle"}</h2>
            <div className="mb-2">
            ...
            <div className="flex justify-end mt-4">
              <Button type="submit" className="w-full bg-sky-500" disabled={loading}>{editing ? "Update" : "Add"}</Button>
            </div>
          </form>
        </div>
      )}
